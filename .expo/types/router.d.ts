...
export namespace ExpoRouter {
  type StaticRoutes = `/` | `/(authenticated)/` | `/(authenticated)/(modals)/account` | `/(authenticated)/(tabs)` | `/(authenticated)/account` | `/(modals)/account` | `/(tabs)` | `/_sitemap` | `/account` | `/crypto` | `/home` | `/invest` | `/lifestyle` | `/login` | `/signUp` | `/transfers`;
  type DynamicRoutes<T extends string> = `/(authenticated)/crypto/${SingleRoutePart<T>}` | `/crypto/${SingleRoutePart<T>}` | `/verify/${SingleRoutePart<T>}`;
  type DynamicRouteTemplate = `/(authenticated)/crypto/[id]` | `/crypto/[id]` | `/verify/[phone]`;
